		...
		#endif
	{
		...
		public void OnPreprocessBuild(BuildTarget target, string path)
		{
			if (target == BuildTarget.Android)
			{
				...
				PlayerSettings.GetScriptingDefineSymbols(NamedBuildTarget.Android, out string[] definesArray);
	#else
			...
			if (target == BuildTarget.iOS)
			{
				...
				if (indexMetal < 0)
				{
					...
					message += "\n\nPlease go to 'Player Settings' > 'Graphics APIs' and add Metal to the top of the list.";
				...
				if (indexOpenGLES2 >= 0 && indexMetal >= 0 && indexOpenGLES2 < indexMetal)
				{
					...
					message += "\n\nPlease go to 'Player Settings' > 'Graphics APIs' remove OpenGLES 2.0 add Metal to the top of the list.";
					ShowAbortDialog(message);
				}
#endif
				...
				if (indexOpenGLES3 >= 0 && indexMetal >= 0 && indexOpenGLES3 < indexMetal)
				{
					...
					message += "\n\nPlease go to 'Player Settings' > 'Graphics APIs' remove OpenGLES 3.0 and add Metal to the top of the list.";
					ShowAbortDialog(message);
				}
