...
public class DoPoints : MonoBehaviour
{
    ...
    public void PreparePoints()
    {
        ...
        {
            for (float horz = -21; horz <= 21; horz += 7)
            {
                for (float vert = -21; vert <= 21; vert += 7)
                {
    ...
    public void Group()
    {
        SetsByDistance.Clear();

        foreach (var point in points)
        {
            float key = Mathf.Max(Mathf.Abs(point.x), Mathf.Abs(point.y));
            if (!SetsByDistance.ContainsKey(key))
            {
                SetsByDistance[key] = new List<Vector3>();
            }
            SetsByDistance[key].Add(point);
        }


        foreach (var set in SetsByDistance)
        {
            Debug.Log($"Key: {set.Key}, Points ({set.Value.Count}): {string.Join(",", set.Value)}");
        }
    ...
    public void BuildSets()
    {
        ...
        foreach (var set in SetsByDistance)
        {
            ints.Add(set.Value.Count);
        }
        var lcm = LCM(ints);

        Debug.Log($"LCM: {lcm}");

        List<Vector3> equalProbability = new List<Vector3>();

        foreach (var set in SetsByDistance)
        {
            var count = set.Value.Count;
            var ratio = lcm / count;
            for (int i = 0; i < ratio; i++)
            {
                equalProbability.AddRange(set.Value);
            }
        }

        equalProbability.Shuffle();

        Dictionary<Vector2, HashSet<float>> counts = new();
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            while (set.Count < 100)
            {
                var point = equalProbability[UnityEngine.Random.Range(0, equalProbability.Count)];


                var coordXY = new Vector2(point.x, point.y);
                if (!counts.ContainsKey(coordXY))
                {
                    counts[coordXY] = new HashSet<float>();
                }
                if (counts[coordXY].Contains(point.z))
                    continue;
                if (counts[coordXY].Count > 2)
                    continue;
                counts[coordXY].Add(point.z);
        ...
        {
            Debug.Log($"Set ({set.Count}): {string.Join(",", set)}");
        }

        for (int i = 0; i < RandomSets.Count; i++)
        {
            for (int j = i + 1; j < RandomSets.Count; j++)
            {
                Debug.Log($"Overlap between {i} and {j}: {GetOverlap(RandomSets[i], RandomSets[j])}");
            }
        }
        ...
        {
            indices.Clear();
            foreach (var point in RandomSets[i])
            {
