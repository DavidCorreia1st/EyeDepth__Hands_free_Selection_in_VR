...
public class VarjoVergengeHandlerPrototype : MonoBehaviour
{
    ...
    private SelectionMethod selectionMethod = SelectionMethod.RayCast;
    private ConfirmationMethod confirmationMethod = ConfirmationMethod.DoubleBlinking;
    ...
    void Start()
    {
        ...
        //check how many frames are used for the mean calculated results
        textMesh.transform.position = new Vector3(0f, 1.3f, 1.5f);
        textMesh.text = "Frame Count: " + meanPositionCount.ToString();
    ...
    void Update()
    {
        textMesh.text = "Frame Count: " + meanPositionCount.ToString() + "ConeCast hits" + miniClones.Count.ToString();
        ...
        if (IsGazeAvailable())
        {
            ...
            for (int i = 0; i < numberOfRecords; i++)
            {
                ...
                if (sphereOverlapRadius < 0.1f) sphereOverlapRadius = 0.1f;
                ...
                HashSet<Material> currentHitMaterials = new HashSet<Material>();

                //METHOD 1
                if (selectionMethod == SelectionMethod.OverlapSphere)
                {
                    Collider[] colliders = Physics.OverlapSphere(positionCurrent, sphereOverlapRadius);
                    MeshRenderer closestTarget = closestTargetToVergencePoint3D(colliders, positionCurrent);
                    //sphere used to comfirm the detection method is working properly
                    debugSphere.position = positionCurrent;
                    ...
                    if (currentEyeState == EyeState.Open && !blinkedOnce && keepTracking)
                    {
                        if (closestTarget != null)
                        {
                            ...
                            {
                                if (closestTarget != lastSelectedTarget)
                                {
                                    material.SetColor("_Color", on);
                ...
                if (selectionMethod == SelectionMethod.SphereCast)
                {
                    ...
                    Vector3 startPosition = positionCurrent - direction * halfDistance;
                    RaycastHit[] hits = Physics.SphereCastAll(startPosition, sphereCastRadius, direction, halfDistance * 2);
                ...
                if (selectionMethod == SelectionMethod.ConeCast)
                {
                    ...
                    if (!coneCastLocked)
                    {
                        ...
                        if (currentEyeState == EyeState.Open && !blinkedOnce && keepTracking)
                        {
                            ...
                            float halfDistance = getErrorCalculatedDistance(calculatedDistance, true);
                            float fullDistance = (calculatedDistance + halfDistance) * 1.3f;
    ...
    private void resetObjectColors(HashSet<Material> currentHitMaterials)
    {
        ...
        foreach (Transform selectedObject in selectedObjects)
        {
            ...
            {
                meshRenderer.material.SetColor("_Color", off);
    ...
    private float getErrorCalculatedDistance(float distance, bool isDepth)
    {
        ...
        {
            return Mathf.Max(horizontalErrors[distance], verticalErrors[distance]);
        }
    ...
    private void placeCloneObjects(Vector3 coneOrigin, Vector3 gazeDirection)
    {
        ...
        bool applyDepthScaling = depthRange > 0.2f;
        ...
        foreach (GameObject coneHit in coneHits)
        {
            ...
            if (originalHit.color != confirmed)
            {
            ...
            float cloneRadius = clone.GetComponent<Renderer>().bounds.extents.magnitude;
            ...
            if (rend != null)
            {
                ...
                clone.transform.position += planeNormal * halfDepth * 0.5f;
            }
    ...
    private void exitCircularPlane()
    {
        ...
        {
            if (lastSelectedTarget.gameObject != hit)
            {
                hit.GetComponent<MeshRenderer>().material.SetColor("_Color", off);
                changedMaterials.Remove(hit.GetComponent<MeshRenderer>().material);
            }
        }
    }
    ...
    public void changeSeletion(bool right)
    {
        if (right)
        {
    ...
    private void updateCircularPlane(Vector3 playerPosition, Vector3 gazeDirection)
    {
        if (circularPlane.GetComponent<MeshRenderer>().enabled != false)
        {
            ...
            float angleOffset = 15f; // 15 degrees
            float distance = 0.4f; // 0.4 meters
            ...
            Vector3 targetPosition = Camera.main.transform.position + rotatedDirection * distance;
